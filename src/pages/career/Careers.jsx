...
export default function Careers() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingJob) {
        await axios.put(`${import.meta.env.VITE_API_URL}/admin/jobs/${editingJob._id}`, submissionData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        toast.success('Job updated');
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL}/admin/jobs`, submissionData, {
          headers: { Authorization: `Bearer ${token}` }
        });
